...
export const Dashboard = () => {
  ...
  const featureCards = [
    {
      id: "scan",
      title: t("scanItem") || "Scan Material",
      desc: "Instant AI price estimation",
      icon: MdCameraAlt,
      path: "/scan",
    ...
    {
      id: "prices",
      title: t("todayPrices") || "Today's Prices",
      desc: "Live scrap market rates",
      icon: MdAttachMoney,
      path: "/prices",
    ...
    {
      id: "recyclers",
      title: t("nearbyRecyclers") || "Nearby Buyers",
      desc: "Find verified recyclers",
      icon: MdMap,
      path: "/recyclers",
    ...
    {
      id: "safety",
      title: t("safetyGuide") || "Safety Tips",
      desc: "Hazardous handling rules",
      icon: MdSecurity,
      path: "/safety",
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <main className="max-w-md mx-auto p-4 space-y-5">
        ...
        >
          <div className="relative z-10">
            <span className="bg-white/20 text-white text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
              {t("greeting") || "Namaste, Ramesh!"}
            </span>
            <h2 className="text-2xl font-extrabold mt-2 leading-tight">
              Ready to sell scrap today?
            </h2>
            <p className="text-emerald-100 text-xs mt-1 font-medium">
              {t("subtitle") || "Check rates & view nearby buyers in Dharavi"}
            </p>
          </div>
          <div className="absolute right-[-10px] bottom-[-20px] text-8xl opacity-15 select-none">
            ♻️
          </div>
        </motion.div>
        ...
        {/* Big Grid Feature Cards */}
        <div>
          <h3 className="text-xs font-extrabold text-gray-500 uppercase tracking-wider mb-3 px-1">
            Quick Features
          </h3>
          <div className="grid grid-cols-2 gap-3">
            {featureCards.map((card) => (
              <motion.div
                ...
                className={`bg-gradient-to-br ${card.bgGradient} p-4 rounded-3xl ${card.textColor} shadow-md flex flex-col justify-between cursor-pointer min-h-[120px] transition-all`}
              >
        ...
        {/* Live Top Rates Ticker */}
        <div>
          <div className="flex items-center justify-between mb-2.5 px-1">
            <h3 className="text-xs font-extrabold text-gray-500 uppercase tracking-wider">
              Top Scrap Rates Today
            </h3>
            <button
              onClick={() => navigate("/prices")}
              className="text-xs font-bold text-emerald-700 flex items-center hover:underline"
            >
          ...
          <div className="space-y-2.5">
            {mockMaterials.slice(0, 3).map((mat) => (
              <Card
                key={mat.id}
                onClick={() => navigate("/prices")}
                className="p-3.5 hover:border-emerald-200"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-500 font-medium">Updated 10m ago</p>
                    </div>
                  </div>
